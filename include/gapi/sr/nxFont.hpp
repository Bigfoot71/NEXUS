#ifndef NEXUS_SR_FONT_HPP
#define NEXUS_SR_FONT_HPP
...
namespace _sr_impl {
    ...
    struct TextureGlyph : _gapi_impl::TextureGlyph<nexus::sr::Context>
    {
        nexus::gfx::Surface surface;
        Uint16 width, height;

        TextureGlyph(nexus::gfx::Surface&& surface);

        float Draw(nexus::sr::Context& ctx, float x, float y, float scale, const nexus::gfx::Color& tint) const override;
        float Draw3D(nexus::sr::Context& ctx, const nexus::math::Vec3& position, float scale, const nexus::gfx::Color& tint) const override;
    };
...
namespace nexus { namespace sr {
    ...
    /**
     * @brief Basic derivation of the gfx::BasicFont class enabling text rendering in a sr::Context with glyph caching.
     */
    class NEXUS_API Font : public Container<_gapi_impl::Font<Context, _sr_impl::TextureGlyph>>
    {
      public:
        /**
