...
shape2D::AABB shape2D::Rectangle::GetAABB() const
{
    const math::Vec2 v1(x, y), v2(x + w, y + h);
    return AABB(v1.Min(v2), v1.Max(v2));
}

bool shape2D::Rectangle::CollisionAABB(const AABB& aabb) const
{
    return aabb.CollisionRectangle(*this);
}

bool shape2D::Rectangle::CollisionPoint(const math::Vec2& point) const
{
    return point.x >= x && point.x <= x+w && point.y >= y && point.y <= y+h;
}

bool shape2D::Rectangle::CollisionLine(const Line& line) const
{
...
bool shape2D::Rectangle::CollisionMesh(const Mesh& mesh) const
{
    return mesh.CollisionRectangle(*this);
}

bool shape2D::Rectangle::CollisionCircle(const Circle& circle) const
{
...
bool shape2D::Rectangle::CollisionEllipse(const Ellipse& ellipse) const
{
    return ellipse.CollisionRectangle(*this);
}

bool shape2D::Rectangle::CollisionPolygon(const Polygon& poly) const
{
    return poly.CollisionRectangle(*this);
}

bool shape2D::Rectangle::CollisionTriangle(const Triangle& tri) const
{
    return tri.CollisionRectangle(*this);
}

bool shape2D::Rectangle::CollisionTriangle(const TriangleF& tri) const
{
    return tri.CollisionRectangle(*this);
}

bool shape2D::Rectangle::CollisionRectangle(const Rectangle& rect) const
{
    return (x + w >= rect.x) && (rect.x + rect.w >= x) && (y + h >= rect.y) && (rect.y + rect.h >= y);
}

bool shape2D::Rectangle::CollisionRectangle(const RectangleF& rect) const
{
    return (x + w >= rect.x) && (rect.x + rect.w >= x) && (y + h >= rect.y) && (rect.y + rect.h >= y);
}
