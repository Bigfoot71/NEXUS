...
void shape3D::Mesh::GenTangents()
{
    ...
    if (indices.empty())
    {
        for (int i = 0; i < numVertices; i += 3)
        {
            ...
            math::Vec3 v21 = vertices[i2] - vertices[i1];
            math::Vec3 v31 = vertices[i3] - vertices[i1];
...
shape3D::Mesh shape3D::Mesh::Polygon(int sides, float radius)
{
    ...
    {
        mesh.vertices[i]    = { 0.0f, 0.0f, 0.0f };
        mesh.vertices[i+1]  = { std::sin(d)*radius, 0.0f, std::cos(d)*radius };
        mesh.vertices[i+2]  = { std::sin(d+dStep)*radius, 0.0f, std::cos(d+dStep)*radius };
        d += dStep;
    }
...
shape3D::Mesh shape3D::Mesh::Plane(float width, float length, int resX, int resZ)
{
    ...
    for (int z = 0; z < resZ; z++)
    {
        ...
        for (int x = 0; x < resX; x++)
        {
            ...
            mesh.vertices[x + z * resX] = { xPos, 0.0f, zPos };
        }
    }
...
shape3D::Mesh shape3D::Mesh::Sphere(float radius, int rings, int slices)
{
    ...
    for (int ring = 0; ring < rings; ring++)
    {
        ...
        for (int slice = 0; slice < slices; slice++)
        {
            ...
            mesh.vertices.push_back(math::Vec3(nx, ny, nz) * radius);
...
shape3D::Mesh shape3D::Mesh::Cylinder(float radius, float height, int slices)
{
    ...
    for (int i = 0; i < slices; ++i)
    {
        ...
        mesh.vertices[slices + i] = { x, -height / 2, z };
...
shape3D::Mesh shape3D::Mesh::Cone(float radius, float height, int slices)
{
    ...
    mesh.vertices[slices] = { 0.0f, height / 2, 0.0f };
...
shape3D::Mesh shape3D::Mesh::Heightmap(const gfx::Surface& heightmap, const math::Vec3& size)
{
    ...
    for (int z = 0; z < mapZ; z++)
    {
        for (int x = 0; x < mapX; x++)
        {
            ...
            mesh.vertices.emplace_back(x * scaleFactor.x, getGray(heightmap, x, z) * scaleFactor.y, z * scaleFactor.z);
            mesh.vertices.emplace_back(x * scaleFactor.x, getGray(heightmap, x, z + 1) * scaleFactor.y, (z + 1) * scaleFactor.z);
            mesh.vertices.emplace_back((x + 1) * scaleFactor.x, getGray(heightmap, x + 1, z) * scaleFactor.y, z * scaleFactor.z);

            mesh.vertices.push_back(mesh.vertices[vCounter + 2]);
            mesh.vertices.push_back(mesh.vertices[vCounter + 1]);
            mesh.vertices.emplace_back((x + 1) * scaleFactor.x, getGray(heightmap, x + 1, z + 1) * scaleFactor.y, (z + 1) * scaleFactor.z);
            ...
            for (int i = 0; i < 6; i += 3)
            {
                const math::Vec3 vA = mesh.vertices[vCounter + i];
                const math::Vec3 vB = mesh.vertices[vCounter + i + 1];
                const math::Vec3 vC = mesh.vertices[vCounter + i + 2];
...
shape3D::Mesh shape3D::Mesh::Cubicmap(const gfx::Surface& cubicmap, const math::Vec3& cubeSize)
{
    ...
    int maxTriangles = cubicmap.GetWidth() * cubicmap.GetHeight() * 12;
    ...
    mesh.vertices.reserve(3*maxTriangles);
    ...
    mesh.numFaces = mesh.vertices.size()/3;
...
bool shape3D::Mesh::CollisionPoint(const nexus::math::Vec3& point, const math::Mat4& meshTransform) const
{
    ...
    for (Uint32 i = 0; i < vertices.size(); i += 3)
    {
        ...
        if (!indices.empty())
        {
            v0 = vertices[indices[i]].Transformed(meshTransform);
            v1 = vertices[indices[i + 1]].Transformed(meshTransform);
            v2 = vertices[indices[i + 2]].Transformed(meshTransform);
        }
        else
        {
            v0 = vertices[i].Transformed(meshTransform);
            v1 = vertices[i + 1].Transformed(meshTransform);
            v2 = vertices[i + 2].Transformed(meshTransform);
        }
...
bool shape3D::Mesh::CollisionMesh(const nexus::shape3D::Mesh& mesh, const math::Mat4& meshTransform) const
{
    for (size_t i = 0; i < vertices.size(); i += 3)
    {
        ...
        for (size_t j = 0; j < mesh.vertices.size(); j += 3)
        {
            ...
            {
                u0 = mesh.vertices[mesh.indices[j]].Transformed(meshTransform);
                u1 = mesh.vertices[mesh.indices[j + 1]].Transformed(meshTransform);
                u2 = mesh.vertices[mesh.indices[j + 2]].Transformed(meshTransform);
            }
            else
            {
                u0 = mesh.vertices[j].Transformed(meshTransform);
                u1 = mesh.vertices[j + 1].Transformed(meshTransform);
                u2 = mesh.vertices[j + 2].Transformed(meshTransform);
            }
